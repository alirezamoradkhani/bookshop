...
async def add_edition(request: Request, edition: EditionCreate,uow = Depends(get_uow),token_data = Depends(get_current_user)):
    return await create_edition(uow=uow,token_data=token_data,edition=edition)

@router.patch("/amount")
@limiter.limit("5/minute")
async def change_amount(request: Request, new_amount: int,edition_id : int,uow = Depends(get_uow),token_data = Depends(get_current_user)):
    return await update_amount(uow=uow,token_data=token_data,new_amount=new_amount,edition_id=edition_id)

@router.patch("/price")
@limiter.limit("5/minute")
async def change_price(request: Request, new_price: int,edition_id : int,uow = Depends(get_uow),token_data = Depends(get_current_user)):
    return await update_price(uow=uow,token_data=token_data,new_price=new_price,edition_id=edition_id)

@router.delete("/delete")
@limiter.limit("5/minute")
async def delete_edition(request: Request, edition_id : int,uow = Depends(get_uow),token_data = Depends(get_current_user)):
    return await remove_edition(uow=uow,token_data=token_data,edition_id=edition_id)

@router.post("/external")
@limiter.limit("5/minute")
async def Import_edition(request: Request, book_id: int,external_edition_title:str,uow = Depends(get_uow),token_data = Depends(get_current_user),provider = Depends(get_openlibrary_provider)):
