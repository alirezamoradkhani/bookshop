...
class EditionRepository(MongoRepository):
    ...
    async def update_amount(self, edition: Edition, new_amount: int):
        return await self._set_fields(edition, amount=new_amount)

    async def change_amount(self, edition: Edition, change: int):
        return await self._increment_fields(edition, amount=change)

    async def update_price(self, edition: Edition, new_price: int):
        return await self._set_fields(edition, price=new_price)

    async def soft_delete(self, edition: Edition):
        return await self._set_fields(edition, is_deleted=True)
